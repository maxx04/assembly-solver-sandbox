...
| 3 | `objectPartMap`/`getJoints()`-Rückgabetyp umstellen | 🟡 **teilweise** (3.1 + 3.1b/c erledigt, 3.2/3.3 offen) | `getJoints()` liefert seit 2026-09-11 `JointRef{joint, nestingPrefix}` statt rohem `DocumentObject*` (Teilschritt 3.1, reiner Typ-Umbau, verhaltensneutral, volle 15er-Testmatrix verifiziert). `objectPartMap` selbst bleibt bewusst über `DocumentObject*` geschlüsselt (kein `(obj, subPath)` - würde denselben Körper bei mehreren Joint-Anschlusspunkten in mehrere `ASMTPart`-Instanzen zerfallen lassen). Teilschritt 3.1b/c (ebenfalls 2026-09-11): die zuvor unbedingt laufenden `FCPROJECT-DEBUG`-Logs in `getJoints()`, `isPartConnected()` und `getMovingPartFromSel()` (letztere beide um einen `verboseLog`-Parameter mit Default `false` ergänzt) laufen jetzt nur noch bei Bedarf - relevant, da alle drei Funktionen aus dem interaktiven `preDrag()`/Selektions-Heisspfad heraus pro Mausereignis aufgerufen werden. Ebenfalls verhaltensneutral, volle 15er-Testmatrix verifiziert. Die eigentliche `objectPartMap`-Konsolidierung (Teilschritt 3.2) und ein exploratives `getGlobalPlacement()`-Experiment (3.3, Verdacht auf Mitverursacher des 180°-Rests) stehen noch aus - siehe Umsetzungsplan. |
