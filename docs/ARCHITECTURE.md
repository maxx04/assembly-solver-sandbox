...
| 4 | Kopier-Pipeline entfernen (`synchronizeJoints()` → `ensureNoJointGroup()` auch im Flexibel-Zweig) | 🟡 **teilweise** (4.2 erledigt, 4.1/4.3-4.5 offen) | `AssemblyLink::synchronizeJoints()` kopiert unverändert (nur der `subJoints`-Flag-Wert wurde zwischenzeitlich hin- und hergedreht, siehe `freecad-assembly-link-subjoints-revert.patch`). **Teilschritt 4.2 (2026-09-11):** `ViewProviderAssemblyLink::claimChildren()` neu eingeführt - zeigt für eine flexible `AssemblyLink` die ECHTE `JointGroup` der verlinkten `AssemblyObject` als zusätzliches Baum-Kind (kein Klon), mit Doppel-Anzeige-Schutz (`if (getJointGroup(assemblyLink)) return children;` - solange die Kopie noch existiert, bleibt die Basisklassen-Anzeige unverändert). Rein additiv, verhaltensneutral für den Solver, volle Testmatrix ohne Abweichung; da die Tests unter Xvfb mit voller Gui-Binary laufen, wurde die neue Methode bei jedem `*_nested_flex`-Testfall real durchlaufen (kein Absturz) - impliziter Rauchtest. **Teilschritt 4.1 bewusst zurückgestellt** (nicht wie ursprünglich geplant vor 4.2 gemacht): `AssemblyLink.cpp:227` (`redrawJointPlacements(getJoints())` beim Rigid→Flexibel-Übergang) auf die echten Joints umzustellen, BEVOR die Kopie-Pipeline (4.3) tatsächlich abgeschaltet ist, würde die dort noch sichtbaren lokalen Joint-Kopien mit veralteten JCS-Markierungen zurücklassen (die Platzierung wird an dieser Stelle gerade für die LOKALEN Spiegel-Kopien neu gesetzt, `redrawJointPlacement()` liest aber pro Joint unabhängig die aktuellen Reference1/2-Platzierungen - ein Umstieg jetzt hätte nur die unveränderten echten Objekte neu gezeichnet, nicht die tatsächlich bewegten Kopien) - wird zusammen mit 4.3 gemacht, wo der Kontextwechsel stimmig ist. 4.3 (eigentliche Abschaltung), 4.4 (Python-API), 4.5 (Cleanup) weiterhin offen. |
