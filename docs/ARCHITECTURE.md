...
| 3 | `objectPartMap`/`getJoints()`-Rückgabetyp umstellen | 🟡 **teilweise** (3.1 erledigt, 3.2/3.3 offen) | `getJoints()` liefert seit 2026-09-11 `JointRef{joint, nestingPrefix}` statt rohem `DocumentObject*` (Teilschritt 3.1, reiner Typ-Umbau, verhaltensneutral, volle 15er-Testmatrix verifiziert). `objectPartMap` selbst bleibt bewusst über `DocumentObject*` geschlüsselt (kein `(obj, subPath)` - würde denselben Körper bei mehreren Joint-Anschlusspunkten in mehrere `ASMTPart`-Instanzen zerfallen lassen); die eigentliche Konsolidierung (Teilschritt 3.2) und ein exploratives `getGlobalPlacement()`-Experiment (3.3, Verdacht auf Mitverursacher des 180°-Rests) stehen noch aus - siehe Umsetzungsplan. |
