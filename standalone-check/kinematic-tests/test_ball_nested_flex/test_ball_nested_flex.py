"""
Stufe 2, Testfall 5: EINE Ebene Verschachtelung ueber eine FLEXIBLE Assembly::AssemblyLink,
mit Ball als INNEREM und Fixed als AEUSSEREM Joint (aeusserer Joint ist per Konvention immer
...
Struktur (siehe nested_test_utils.py fuer die volle Herleitung/Nomenklatur):
    Sub (eigenes Dokument):     BoxA (GEERDET, interner Referenzpunkt) -[innerer Ball-Joint]- BoxB
    GrandTop (eigenes Dokument): BoxC (geerdet) -[aeusserer Fixed-Joint]- SubLink.BoxB (Spiegel)

Der innere Ball-Joint hat 3 freie Freiheitsgrade (komplette Rotation) - wie beim flachen
Ball-Test wird dafuer NICHT die exakte Placement-Kette geprueft (waere keine
solver-unabhaengige analytische Referenz mehr, siehe joint_test_utils.py), sondern die
Zwangsbedingung selbst (`jtu.check_ball()`). Der aeussere Fixed-Joint hat 0 Freiheitsgrade -
...
Aufruf (mit der jeweiligen FreeCAD-Binary via run-test.sh):
    ./common/run-test.sh <freecad-install-dir> test_ball_nested_flex/test_ball_nested_flex.py
...
TEST_NAME = "test_ball_nested_flex"

# Echte, aus der Edge9-Geometrie abgeleitete JCS-Placements fuer den INNEREN Ball-Joint
...
INNER_PLC1 = jtu.vertex_jcs_placement("Vertex2", extra_rotation_axis=App.Vector(0, 1, 0), extra_rotation_deg=30)  # BoxA
INNER_PLC2 = jtu.vertex_jcs_placement("Vertex2", extra_rotation_axis=App.Vector(1, 0, 0), extra_rotation_deg=15)  # BoxB
...
def main():
    ...
    def full_check(label, boxC_plc, mirror_boxA_plc, mirror_boxB_plc):
        ...
        rel = jtu.report(f"{label} (innerer Ball)", jcs1, jcs2)
        ok_inner = jtu.check_ball(rel)
    ...
    if ok_outer and ok_inner and ok_sync and ok_sync_reload and ok_outer_reload and ok_inner_reload:
        ...
        if not ok_inner:
            reasons.append("innerer Ball-Joint direkt nach dem Solve verletzt")
        if not ok_sync:
        ...
        if not ok_inner_reload:
            reasons.append("innerer Ball-Joint nach Neuladen verletzt")
