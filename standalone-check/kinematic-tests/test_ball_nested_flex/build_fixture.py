"""
build_fixture.py - baut die feste Ausgangsdatei fuer test_ball_nested_flex.py EINMALIG und
speichert sie git-getrackt unter fixtures/{sub,grand}.FCStd (relativ zu diesem eigenen
Testordner). Analog zu test_fixed_nested_flex/build_fixture.py, nur mit Ball statt Fixed als
...
from test_ball_nested_flex import INNER_PLC1, INNER_PLC2, OUTER_PLC1, OUTER_PLC2
...
def main():
    ...
    sub_doc, sub_asm, subA, subB = ntu.new_sub_assembly_doc("BallNestedFlexSub", sub_path)
    jtu.make_joint(sub_asm, 4, subA, subB, INNER_PLC1, INNER_PLC2, elem1="Vertex2", elem2="Vertex2")  # Ball
    ...
    # vorbelegt (kein bewusster "Treiber/Drag"-Testfall) - Ball hat 3 freie Freiheitsgrade (komplette Rotation)
    ...
    grand_doc, grand_asm, boxC, sublink = ntu.new_grand_assembly_with_sublink(
        "BallNestedFlexGrand", sub_asm, sub_path, grand_path
    )
