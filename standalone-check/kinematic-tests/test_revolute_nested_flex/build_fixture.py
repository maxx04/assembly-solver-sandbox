"""
build_fixture.py - baut die feste Ausgangsdatei fuer test_revolute_nested_flex.py EINMALIG und
speichert sie git-getrackt unter fixtures/{sub,grand}.FCStd (relativ zu diesem eigenen
Testordner). Analog zu test_fixed_nested_flex/build_fixture.py, nur mit Revolute statt Fixed als
...
from test_revolute_nested_flex import INNER_PLC1, INNER_PLC2, OUTER_PLC1, OUTER_PLC2
...
def main():
    ...
    # vorbelegt (kein bewusster "Treiber/Drag"-Testfall) - Revolute hat 1 freien Freiheitsgrad
    ...
    grand_doc, grand_asm, boxC, sublink = ntu.new_grand_assembly_with_sublink(
        "RevoluteNestedFlexGrand", sub_asm, sub_path, grand_path
    )
