"""
Stufe 3, Testfall 3: ZWEI Ebenen Verschachtelung (Sub -> Mid -> GrandTop), mit Cylindrical
...
Direkte Erweiterung von test_cylindrical_nested_flex.py (Stufe 2) um eine dritte Ebene - siehe
...
Struktur:
    Sub (eigenes Dokument):  BoxA (GEERDET)          -[innerster Cylindrical-Joint]-  BoxB
...
Der innerste Cylindrical-Joint hat einen freien Freiheitsgrad - dafuer wird (wie beim flachen
und beim Stufe-2-Cylindrical-Test) NICHT die exakte Placement-Kette geprueft, sondern die
Zwangsbedingung selbst (`jtu.check_cylindrical()`). Mittlerer+aeusserer Joint haben 0 Freiheitsgrade -
...
korrekt konvergieren; der innerste Cylindrical-Joint kann denselben bekannten 180-Grad-Rest
...
Aufruf:
    ./common/run-test.sh <freecad-install-dir> test_cylindrical_double_nested_flex/test_cylindrical_double_nested_flex.py
"""
import os
import sys
import shutil
...
TEST_NAME = "test_cylindrical_double_nested_flex"
FIXTURES_DIR = os.path.join(THIS_DIR, "fixtures")

# INNER_PLC* identisch zu test_cylindrical_nested_flex.py (Stufe 2); MIDDLE_PLC*/OUTER_PLC*
...
def _as_obj(plc):
    ...
    Adapter (siehe test_cylindrical_nested_flex.py)."""
    class _Wrapper:
        pass
    w = _Wrapper()
    w.Placement = plc
...
def full_check(label, boxC_plc, boxD_mirror_plc, mirror_boxA_plc, mirror_boxB_plc):
    ...
    rel = jtu.report(f"{label} (innerster Cylindrical)", jcs1, jcs2)
