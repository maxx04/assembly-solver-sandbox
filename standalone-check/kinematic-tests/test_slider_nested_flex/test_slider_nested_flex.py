"""
Stufe 2, Testfall 2: EINE Ebene Verschachtelung ueber eine FLEXIBLE Assembly::AssemblyLink,
mit Slider als INNEREM und Fixed als AEUSSEREM Joint (aeusserer Joint ist per Konvention immer
...
Struktur (siehe nested_test_utils.py fuer die volle Herleitung/Nomenklatur):
    Sub (eigenes Dokument):     BoxA (GEERDET, interner Referenzpunkt) -[innerer Slider-Joint]- BoxB
    GrandTop (eigenes Dokument): BoxC (geerdet) -[aeusserer Fixed-Joint]- SubLink.BoxB (Spiegel)

Der innere Slider-Joint hat 1 freien Freiheitsgrad (Translation entlang der Kantenachse) - wie
beim flachen Slider-Test wird dafuer NICHT die exakte Placement-Kette geprueft (waere keine
solver-unabhaengige analytische Referenz mehr, siehe joint_test_utils.py), sondern die
Zwangsbedingung selbst (`jtu.check_slider()`). Der aeussere Fixed-Joint hat 0 Freiheitsgrade -
...
Aufruf (mit der jeweiligen FreeCAD-Binary via run-test.sh):
    ./common/run-test.sh <freecad-install-dir> test_slider_nested_flex/test_slider_nested_flex.py
...
TEST_NAME = "test_slider_nested_flex"

# Echte, aus der Edge9-Geometrie abgeleitete JCS-Placements fuer den INNEREN Slider-Joint
...
def main():
    ...
    def full_check(label, boxC_plc, mirror_boxA_plc, mirror_boxB_plc):
        ...
        rel = jtu.report(f"{label} (innerer Slider)", jcs1, jcs2)
        ok_inner = jtu.check_slider(rel)
    ...
    if ok_outer and ok_inner and ok_sync and ok_sync_reload and ok_outer_reload and ok_inner_reload:
        ...
        if not ok_inner:
            reasons.append("innerer Slider-Joint direkt nach dem Solve verletzt")
        if not ok_sync:
        ...
        if not ok_inner_reload:
            reasons.append("innerer Slider-Joint nach Neuladen verletzt")
