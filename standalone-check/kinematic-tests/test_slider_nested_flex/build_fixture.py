"""
build_fixture.py - baut die feste Ausgangsdatei fuer test_slider_nested_flex.py EINMALIG und
speichert sie git-getrackt unter fixtures/{sub,grand}.FCStd (relativ zu diesem eigenen
Testordner). Analog zu test_fixed_nested_flex/build_fixture.py, nur mit Slider statt Fixed als
...
from test_slider_nested_flex import INNER_PLC1, INNER_PLC2, OUTER_PLC1, OUTER_PLC2
...
def main():
    ...
    sub_doc, sub_asm, subA, subB = ntu.new_sub_assembly_doc("SliderNestedFlexSub", sub_path)
    jtu.make_joint(sub_asm, 3, subA, subB, INNER_PLC1, INNER_PLC2, elem1="Edge9", elem2="Edge9")  # Slider
    ...
    # vorbelegt (kein bewusster "Treiber/Drag"-Testfall) - Slider hat 1 freien Freiheitsgrad
    ...
    grand_doc, grand_asm, boxC, sublink = ntu.new_grand_assembly_with_sublink(
        "SliderNestedFlexGrand", sub_asm, sub_path, grand_path
    )
