"""
build_fixture.py - baut die feste Ausgangsdatei fuer test_ball_double_nested_flex.py EINMALIG
...
Aufruf:
    ./common/run-test.sh /home/maxx/freecad-sandbox/install test_ball_double_nested_flex/build_fixture.py
"""
import os
import sys
...
from test_ball_double_nested_flex import (
...
def main():
    ...
    mid_doc, mid_asm, boxD, sublink = ntu.new_grand_assembly_with_sublink(
        "BallDoubleNestedFlexMid", sub_asm, sub_path, mid_path, box_name="BoxD", link_name="SubLink"
    )
    ...
    grand_doc, grand_asm, boxC, midlink = ntu.new_grand_assembly_with_sublink(
        "BallDoubleNestedFlexGrand", mid_asm, mid_path, grand_path, box_name="BoxC", link_name="MidLink"
    )
